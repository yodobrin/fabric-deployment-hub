...
{
public static async Task<IDeploymentRequest?> CreateDeploymentRequestAsync(PlatformMetadata metadata, 
    string folder, 
    Guid workspaceId, 
    ...
    ILogger logger)
{
    return metadata.Metadata.Type switch
    {
        "Notebook" => new DeployNotebookRequest
        {
            ...
            {
                Parts = await BlobUtils.GetItemPartsAsync(blobContainerClient,
                                folder,
                                metadata.Metadata.Type,
                                logger,
                                parameters,secrets,settings)
            }
        },
        _ => null
    };
