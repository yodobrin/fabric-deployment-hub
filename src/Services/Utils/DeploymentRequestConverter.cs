...
{
    public override IDeploymentRequest? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        return type switch
        {
            "notebook" => JsonSerializer.Deserialize<DeployNotebookRequest>(jsonObject.GetRawText(), options),
            // Add other types here...
            _ => throw new NotSupportedException($"Unknown deployment request type: {type}")
        };
    }

    public override void Write(Utf8JsonWriter writer, IDeploymentRequest value, JsonSerializerOptions options)
    {
