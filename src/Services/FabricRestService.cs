...
public class FabricRestService : IFabricRestService
{
    ...
    public FabricRestService(HttpClient httpClient, ITokenService tokenService, ILogger<FabricRestService> logger)
    {
    ...
    private async Task<string> ExecuteWithCompletionHandling(HttpMethod method, string uri, object? payload, bool waitForCompletion, TimeSpan? pollingInterval, TimeSpan? timeout)
    {
        ...
        HttpRequestMessage request = new(method, compoundUri)
        {
            Content = payload != null
                ? new StringContent(JsonSerializer.Serialize(payload, new JsonSerializerOptions { WriteIndented = true }), Encoding.UTF8, "application/json")
                : null
        };
        ...
        try
        {
            ...
            _logger.LogInformation("Sending {Method} request to {Uri} with payload: {Payload}", method, uri, JsonSerializer.Serialize(sanitizedPayload, new JsonSerializerOptions { WriteIndented = true }));

            var response = await _httpClient.SendAsync(request);
            if (!response.IsSuccessStatusCode)
            {
                string responseContent = await response.Content.ReadAsStringAsync();
                _logger.LogWarning("{Method} request to {Uri} failed with status {StatusCode}. Response: {Response}", method, uri, response.StatusCode, responseContent);
            }
            response.EnsureSuccessStatusCode();
    ...
    private async Task<string> WaitForCompletionAsync(string location, TimeSpan pollingInterval, TimeSpan timeout)
    {
        try
        {
            ...
            while (DateTime.UtcNow - startTime < timeout)
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    _logger.LogInformation("Operation at {Location} completed successfully.", location);
                    return await response.Content.ReadAsStringAsync();
                }

                if (response.StatusCode != HttpStatusCode.Accepted)
                {
                    _logger.LogWarning("Unexpected status code {StatusCode} during polling for {Location}.", response.StatusCode, location);
            ...
            throw new TimeoutException($"Operation did not complete within the specified timeout of {timeout.TotalSeconds} seconds.");
        }
        catch (Exception ex)
        {
    ...
    private async Task<string> HandleNonWaitingScenario(HttpResponseMessage response)
    {
        ...
        {
            _logger.LogInformation("Response includes location header: {Location}", response.Headers.Location);
        ...
        {
            _logger.LogWarning("Unexpected response status code: {StatusCode}", response.StatusCode);
    ...
    private void LogResponse(HttpResponseMessage response, string method, string uri)
    {
        if (response.IsSuccessStatusCode)
        {
            _logger.LogInformation("{Method} request to {Uri} succeeded with status code {StatusCode}.", method, uri, response.StatusCode);
        }
        else
        {
            _logger.LogWarning("{Method} request to {Uri} failed with status code {StatusCode}.", method, uri, response.StatusCode);
        }
    }
}