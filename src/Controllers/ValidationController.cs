...
public class ValidationController : ControllerBase
{
    ...
    public async Task<IActionResult> ValidatePlan([FromBody] TenantDeploymentRequest request)
    {
        if (request == null || string.IsNullOrEmpty(request.PlanFile) || string.IsNullOrEmpty(request.RepoContainer))
        {
            ...
            return BadRequest(new { message = "Request body is missing or invalid. Please provide PlanFile and RepoContainer." });
        }

        _logger.LogInformation("Starting validation for plan file: {PlanFile} in container {RepoContainer}", request.PlanFile, request.RepoContainer);
        ...
        {
            _logger.LogError(ex, "Error loading deployment plan from {PlanFile} in container {RepoContainer}.", request.PlanFile, request.RepoContainer);
            return StatusCode(500, new { message = "An error occurred while loading the deployment plan." });
        }
        ...
        foreach (var workspaceDeploymentPlan in tenantDeploymentPlan.Workspaces)
        {
            try
            {
                var items = await _fabricRestService.GetWorkspaceItemsAsync(workspaceDeploymentPlan.WorkspaceId);

                foreach (var deploymentRequest in workspaceDeploymentPlan.DeploymentRequests)
                {
                    // in fabric its all by name
                    var existingItem = items.FirstOrDefault(i => i.DisplayName == deploymentRequest.DisplayName);
            ...
            {
                _logger.LogError(httpEx, "Error fetching items for workspace {WorkspaceId}.", workspaceDeploymentPlan.WorkspaceId);
                foreach (var deploymentRequest in workspaceDeploymentPlan.DeploymentRequests)
                {
            ...
            {
                _logger.LogError(ex, "Unhandled error during validation for workspace {WorkspaceId}.", workspaceDeploymentPlan.WorkspaceId);
                foreach (var deploymentRequest in workspaceDeploymentPlan.DeploymentRequests)
                {
                    deploymentRequest.Validation = "Error";
                }
        ...
        try
        {
            ...
            _logger.LogInformation("Validated deployment plan saved as {ValidatedPlanFileName} in container {RepoContainer}.", validatedPlanFileName, request.RepoContainer);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to save validated deployment plan.");
            return StatusCode(500, new { message = "An error occurred while saving the validated deployment plan." });
        }
